    ...
    try:
        ...
        for date in dates:
            ...
            for pollster in selected_pollsters:
                ...
                parties = [max(1, val) for val in [base_con, base_lab, base_lib, base_ref, base_grn, base_snp]]
...
def display_latest_averages(df):
    ...
    try:
            ...
            with cols[col_index]:
                ...
                if len(df) >= 20:
                    ...
                    if not older_polls.empty:
                        ...
                        trend = "↗️" if avg_val > older_avg + 0.5 else "↘️" if avg_val < older_avg - 0.5 else "→"
                    else:
                        trend = "→"
                else:
                    trend = "→"
                ...
                st.markdown(
                    f"""<div class="party-metric" style="border-left: 4px solid {party_colors[party]};">
        ...
        for party in party_columns:
            trend_data[f"{party}_avg"] = trend_data[party].rolling(window=3, min_periods=1).mean()
        
        # Display chart
        chart_data = trend_data.set_index("Date")[[f"{party}_avg" for party in party_columns]]
        ...
        st.info("Unable to calculate averages. This might be due to insufficient or malformed data.")
...
def main():
    ...
    st.markdown(
        ...
        <small style="color: #999;">Sprint 1: Enhanced Sample Data Display & UI Components</small>
        </p>''', 
        unsafe_allow_html=True
    )
    ...
    with st.sidebar:
        ...
        st.success("**Sprint 1, Day 2** - Enhanced UI & Data Display")
    ...
    try:
        ...
        if 'Pollster' in poll_data.columns:
            available_pollsters = ["All Pollsters"] + sorted(poll_data['Pollster'].unique().tolist())
        ...
        st.markdown(
            f'''<div class="success-message">
                ✅ Successfully loaded {len(filtered_data)} polls from {filtered_data['Pollster'].nunique()} pollsters
            </div>''', 
            unsafe_allow_html=True
        )
        ...
        # Add party columns
        party_columns = ["Conservative", "Labour", "Liberal Democrat", "Reform UK", "Green", "SNP", "Others"]
        columns_to_show.extend([col for col in party_columns if col in display_data.columns])
        ...
        try:
            ...
            with col1:
                ...
                if 'Sample Size' in filtered_data.columns:
                    ...
                    sample_quality = "High" if avg_sample > 1500 else "Medium" if avg_sample > 1000 else "Low"
                    st.metric("Average Sample Size", f"{avg_sample:.0f}", help=f"Quality: {sample_quality}")
            
            with col2:
                # Pollster diversity
                pollster_count = filtered_data['Pollster'].nunique()
                diversity = "High" if pollster_count >= 5 else "Medium" if pollster_count >= 3 else "Low"
                st.metric("Pollster Diversity", pollster_count, help=f"Diversity: {diversity}")
            
            with col3:
                # Data recency
                latest_poll_days = (datetime.now() - pd.to_datetime(filtered_data['Date'].max())).days
                recency = "Fresh" if latest_poll_days <= 3 else "Moderate" if latest_poll_days <= 7 else "Stale"
                st.metric("Data Freshness", f"{latest_poll_days} days", help=f"Status: {recency}")
                ...
                if not pollster_avg.empty:
                    ...
                    for party in party_columns:
                        if party in pollster_avg.columns:
                            ...
                            st.markdown(f"- **{party}**: {max_pollster} ({max_value}%)")
        except:
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: #666; margin-top: 2rem; padding: 1rem; 
            ...
            <p><strong>UK Election Simulator v0.2.0 - Sprint 1, Day 2</strong><br>
            Enhanced UI & Data Display | Built with Streamlit<br>
            <a href='https://github.com/data-john/Election-Models-UKGE' target='_blank' 
               style='color: #0066cc; text-decoration: none;'>📚 Source Code</a> | 
            <a href='#' style='color: #0066cc; text-decoration: none;'>📖 Documentation</a><br>
